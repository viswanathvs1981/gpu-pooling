...
func main() {
	...
	logger.Info("Starting Deployment Agent", "redis", redisAddr, "mcp_url", mcpServerURL)

	// Create agent
	agent, err := agents.NewDeploymentAgent(redisAddr, mcpServerURL)
	if err != nil {
		logger.Error(err, "Failed to create deployment agent")
		os.Exit(1)
	}
	...
	// Start agent
	logger.Info("Deployment Agent ready")
	if err := agent.Start(ctx); err != nil {
		logger.Error(err, "Deployment Agent stopped with error")
		os.Exit(1)
	}
