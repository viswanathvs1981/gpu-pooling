...
func startCustomResourceController(
	...
	if err := (&controller.AzureGPUSourceReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "AzureGPUSource")
		os.Exit(1)
	}
	...
	if err := (&controller.WorkloadIntelligenceReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "WorkloadIntelligence")
		os.Exit(1)
	}
}
